...
from multiagent.scenarios.scenario_util import get_noise

class ONScenario(BaseScenario):
    def make_world(self):
        world = World()
    ...
    def observation(self, agent, world):
        ...
        if not agent.adversary:
            return np.concatenate([agent.goal_a.state.p_pos - agent.state.p_pos] + entity_pos + other_pos + noise)
        else:
            return np.concatenate(entity_pos + other_pos + noise)
